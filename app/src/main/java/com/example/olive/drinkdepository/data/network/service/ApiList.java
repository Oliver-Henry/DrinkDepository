package com.example.olive.drinkdepository.data.network.service;

/**
 * Created by olive on 02/03/2018.
 */

public class ApiList {
    public static final String BASE_URL = "https://www.thecocktaildb.com/api/json/v1/1/";
    //    public static final String API_KEY = "1";
    public static final String ORDINARY_DRINKS = "filter.php?c=Ordinary_drink";
    public static final String COCKTAILS = "filter.php?c=Cocktail";
    public static final String HOMEMADE_LIQUEURS = "filter.php?c=Homemade_Liqueur";
    public static final String PUNCH_PARTY_DRINKS = "filter.php?c=Punch_/_Party_Drink";
    public static final String DRINK_DETAILS = "lookup.php";
    public static final String INGREDIENTS_LIST = "list.php?i=list";
    public static final String SEARCH_BY_INGREDIENT = "filter.php?i=Vodka";
    public static final String RANDOM_DRINK = "random.php";
}

  //search.php?s=margarita //lookup.php?i=13060

//https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=13060
